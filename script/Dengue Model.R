...
# load dengue dataframe
dc= read.csv("./dc.csv")   
dc$Month <- as.factor(dc$Month)
dc$IBGE6 <- as.factor(dc$IBGE6)
dc$Year  <- as.factor(dc$Year)  
...
dc$Year  <- as.factor(dc$Year) 
dc$State =  as.factor(dc$State)
dc$Month =  as.numeric(dc$Month)


state_monthly$Date  =  as.Date(state_monthly$Date)
state_monthly$State  <- as.factor(state_monthly$State)  

# --------------------plot dengue incidence - just from 3 states------------
ndc<-dc |> dplyr::select(IBGE6, Municipality, State, Population, NumCases, Year, Date, Month)
...
pd <- ggplot(state_monthly, aes(x = Date, y = incidence, colour = State)) +
  geom_line(size = 1.2, alpha = 0.8) +
  scale_x_date(date_breaks = "1 year",
               date_labels = "%Y",
               expand = c(0.02, 0)) +
  scale_y_continuous(breaks = pretty_breaks(n = 6)) +
  labs(title = "Dengue Incidence Rate by State (2017-2024)",
       x = "Year",
...
ddc$offset = log(ddc$Population/100000)

# check number of zeroes - not zero inflated now
table(ddc$NumCases == 0)
...
m_s0 <- gam(NumCases ~ offset(offset) + s(Month, bs="tp", by=State) + Year*State + 
              s(IBGE6, bs="re") + s(pmonth, bs="tp")+ s(tmean_4m, bs="tp")+ s(spi6, bs="tp"),
            family = "nb",
            method = "REML",
            data = ddc)

m_s1 <- gam(NumCases ~ offset(offset) + s(Month, bs="tp", by=State) + Year*State + 
              s(IBGE6, bs="re") + s(pmonth, bs="tp")+ s(tmean_4m, bs="tp") + s(spi6_1m, bs="tp"),
            family = "nb",
            method = "REML",
            data = ddc)

m_s2 <- gam(NumCases ~ offset(offset) + s(Month, bs="tp", by=State) + Year*State + 
              s(IBGE6, bs="re") + s(pmonth, bs="tp")+ s(tmean_4m, bs="tp") + s(spi6_2m, bs="tp"),
            family = "nb",
            method = "REML",
            data = ddc)

m_s3 <- gam(NumCases ~ offset(offset) + s(Month, bs="tp", by=State) + Year*State + 
              s(IBGE6, bs="re") + s(pmonth, bs="tp")+ s(tmean_4m, bs="tp") + s(spi6_3m, bs="tp"),
            family = "nb",
            method = "REML",
            data = ddc)

m_s4 <- gam(NumCases ~ -1+ offset(offset) + s(Month, bs="tp", by=State) + Year*State + 
              s(IBGE6, bs="re") + s(pmonth, bs="tp")+ s(tmean_4m, bs="tp") + s(spi6_4m, bs="tp"),
            family = "nb",
            method = "REML",
            data = ddc)

m_s5 <- gam(NumCases ~ offset(offset) + s(Month, bs="tp", by=State) + Year*State + 
              s(IBGE6, bs="re") + s(pmonth, bs="tp")+ s(tmean_4m, bs="tp") + s(spi6_5m, bs="tp"),
            family = "nb",
            method = "REML",
            data = ddc)

m_s6 <- gam(NumCases ~ offset(offset) + s(Month, bs="tp", by=State) + Year*State + 
              s(IBGE6, bs="re") + s(pmonth, bs="tp")+ s(tmean_4m, bs="tp") + s(spi6_6m, bs="tp"),
            family = "nb",
            method = "REML",
            data = ddc)
...
draw(m_s4, fun=exp, select="s(tmean_4m)", caption=FALSE)+ 
  labs(title="(A) Association between lagged Temperature and Disease Incidence Rate", 
       x="Monthly Mean Temperature with 4-month lag (°C)", y="Incidence Rate Ratio") +
  geom_hline(yintercept=1, lty=2, color="grey40")+
  theme_minimal()+
  theme(plot.title = element_text(hjust=0.5))
ggsave(filename="DENGUE-T.jpg", device="jpg", units="in", width=10, height=4, dpi=600)

draw(m_s4, fun=exp, select="s(pmonth)", caption=FALSE)+ 
  ...
  theme_minimal()+
  theme(plot.title = element_text(hjust=0.5))
ggsave(filename="DENGUE-P0.jpg", device="jpg", units="in", width=10, height=4, dpi=600)

draw(m_s4, fun=exp, select="s(spi6_4m)", caption=FALSE)+ 
  labs(title="(C) Association between SPI-6 and Disease Incidence Rate", 
       x="SPI-6 with 4-month lag effect", y="Incidence Rate Ratio") +
  geom_hline(yintercept=1, lty=2, color="grey40")+
  theme_minimal()+
  theme(plot.title = element_text(hjust=0.5))
ggsave(filename="DENGUE-S4.jpg", device="jpg", units="in", width=10, height=4, dpi=600)

draw(m_s4, fun=exp, select=c("s(Month):StateMato Grosso") , caption=FALSE) + 
  labs(title="Seasonal Variation of Dengue Incidence Rate",
...
draw(m_s4, fun=exp, select=c("s(Month):StateGoiás") , caption=FALSE) + 
  labs(title="Seasonal Variation of Dengue Incidence Rate",
       y="Incidence Rate Ratio") +
  scale_x_continuous(breaks=seq(1,12,1))+
  geom_hline(yintercept=1, lty=2, color="grey40")+
...
draw(m_s4, fun=exp, select=c("s(Month):StateMato Grosso do Sul") , caption=FALSE) + 
  labs(title="Seasonal Variation of Dengue Incidence Rate",
       y="Incidence Rate Ratio") +
  scale_x_continuous(breaks=seq(1,12,1))+
  geom_hline(yintercept=1, lty=2, color="grey40")+
...
draw(m_s4, fun=exp, select=c("s(Month):StateDistrito Federal") , caption=FALSE) +  
  labs(title="Seasonal Variation of Dengue Incidence Rate",
       y="Incidence Rate Ratio") +
  scale_x_continuous(breaks=seq(1,12,1))+
  geom_hline(yintercept=1, lty=2, color="grey40")+
  theme_minimal()
ggsave(filename="DF.jpg", device="jpg", units="in", width=10, height=4, dpi=600)

#========================================================================================
# Example: assign rough population sizes (or use actual means from your data)
nn<-dc |> dplyr::select(IBGE6, State, Region, Municipality, Population, Year, Month)
...
new_data <- expand.grid(
  Month = 1:12,
  Year = 2017:2024,
  State = c("Distrito Federal", "Goiás", "Mato Grosso", "Mato Grosso do Sul"),
  IBGE6 = "dummy", # placeholder
  tmean_4m = mean(ddc$tmean_4m),
  pmonth = mean(ddc$pmonth),
  spi6_4m = mean(ddc$spi6_4m)
)
...
# Create the Offset
new_data$offset <- log(new_data$Population / 100000)

# Predict and plot
predictions <- predict(m_s4, newdata=new_data, type="response")
...
ggplot(new_data, aes(x=Month, y=Year, fill=predicted)) +
  ...
  theme_minimal() +
  labs(title="Fitted dengue incidence by month and year across states")
...
year_coefs <- coef(m_s4)[grep("Year", names(coef(m_s4)))]
year_se <- summary(m_s4)$se[grep("Year", names(coef(m_s4)))]
...
save(list = ls(all.names = TRUE), file = "dengue.RData")  